...
def combine_fts_and_kuka_pose(
    ...
    - Aligns timelines using Fz peaks
    ...
    # --- Detect all peaks in FZ and compute time lag ---
    Fz_fts = smooth_series(df_fts["Fz"].to_numpy(), window=25)
    Fz_kuka = smooth_series(df_kuka["FZ"].to_numpy(), window=5)

    # Thresholds
    h_fts = np.median(Fz_fts) + 1.0 * np.std(Fz_fts)
    h_kuka = np.median(Fz_kuka) + 1.0 * np.std(Fz_kuka)
    # Minimum distance in samples (~20 ms)
    dt_fts = float(np.median(np.diff(t_fts_rel))) if len(t_fts_rel) > 1 else 0.001
    min_dist_fts = max(1, int(round(0.02 / max(dt_fts, 1e-6))))
    dt_kuka = float(np.median(np.diff(t_kuka))) if len(t_kuka) > 1 else 0.005
    min_dist_kuka = max(1, int(round(0.02 / max(dt_kuka, 1e-6))))

    peaks_fts = find_all_peaks(Fz_fts, min_height=h_fts, min_distance=min_dist_fts)
    peaks_kuka = find_all_peaks(Fz_kuka, min_height=h_kuka, min_distance=min_dist_kuka)

    # If too few peaks detected (e.g., quiet data), fall back to first-peak alignment using magnitude
    if len(peaks_fts) >= 1 and len(peaks_kuka) >= 1:
        lag = robust_lag_from_peak_times(t_fts_rel[peaks_fts], t_kuka[peaks_kuka])
    else:
        # Fallback: first peak of |F|
        Fmag_fts_tmp = np.linalg.norm(df_fts[["Fx", "Fy", "Fz"]].to_numpy(), axis=1)
        Fmag_kuka_tmp = np.linalg.norm(df_kuka[["FX", "FY", "FZ"]].to_numpy(), axis=1)
        idx_peak_fts = first_peak_index(smooth_series(Fmag_fts_tmp, window=25))
        idx_peak_kuka = first_peak_index(smooth_series(Fmag_kuka_tmp, window=5))
        lag = float(t_fts_rel[idx_peak_fts] - t_kuka[idx_peak_kuka])

    # Shift KUKA time so that peaks align: t_kuka_shifted + 0 ~= t_fts
    t_kuka_shifted = t_kuka + lag
    ...
    # Metadata
    meta: Dict[str, float] = {
        "lag_sec_kuka_to_fts": float(lag),
        "num_peaks_fts": int(len(peaks_fts)),
        "num_peaks_kuka": int(len(peaks_kuka)),
    ...
    if plot:
        ...
        # Row 0: Forces
        plot_row(0, ["Fx", "Fy", "Fz"], ["Fx (FTS)", "Fy (FTS)", "Fz (FTS)"], units="N")
        # Row 1: Torques
        plot_row(1, ["Tx", "Ty", "Tz"], ["Tx (FTS)", "Ty (FTS)", "Tz (FTS)"], units="Nm")
        # Row 2: Position
        ...
        fig.suptitle("FTS Wrench and KUKA Pose (Aligned by FZ Peaks)")
...
def combine_trials_kuka_ati(
    ...
    For each trial value present in both files:
      - Align timelines using peaks in FZ (KUKA FZ vs ATI Fz)
    ...
    if "t_epoch" in df_fts_all.columns:
        ...
        # fallback if ATI file already has seconds
    ...
    for tr in common_trials:
        ...
        # Peak-based lag using FZ
        Fz_fts = smooth_series(df_fts["Fz"].to_numpy(dtype=float), window=25 if len(df_fts) > 50 else 5)
        Fz_kuka = smooth_series(df_kuka["FZ"].to_numpy(dtype=float), window=5)
        ...
        if len(peaks_fts) >= 1 and len(peaks_kuka) >= 1:
            lag = robust_lag_from_peak_times(t_fts_rel[peaks_fts], t_kuka_rel[peaks_kuka])
        else:
            # Fallback to first magnitude peak
            Fmag_fts_tmp = np.linalg.norm(df_fts[["Fx", "Fy", "Fz"]].to_numpy(dtype=float), axis=1)
            Fmag_kuka_tmp = np.linalg.norm(df_kuka[["FX", "FY", "FZ"]].to_numpy(dtype=float), axis=1)
            idx_peak_fts = first_peak_index(smooth_series(Fmag_fts_tmp, window=25 if len(df_fts) > 50 else 5))
            idx_peak_kuka = first_peak_index(smooth_series(Fmag_kuka_tmp, window=5))
            lag = float(t_fts_rel[idx_peak_fts] - t_kuka_rel[idx_peak_kuka])
        ...
        if use_fts_base:
            base_t = t_fts_rel
            # Shift KUKA forward by lag to align with FTS timeline
            t_kuka_shifted = t_kuka_rel + lag
            ...
            base_t = t_kuka_rel
            # Shift FTS backward by lag so that FTS aligns to KUKA timeline
            t_fts_shifted = t_fts_rel - lag
        ...
        if plot:
            ...
            # Position
            axes[2, 0].plot(part["time"], part["X"]); axes[2, 0].set_title("X")
            axes[2, 1].plot(part["time"], part["Y"]); axes[2, 1].set_title("Y")
            axes[2, 2].plot(part["time"], part["Z"]); axes[2, 2].set_title("Z")
            ...
            fig.suptitle("Synchronized (biased) Wrench and Pose")
            fig.tight_layout()
            plt.show()
...
if __name__ == "__main__":
    ...
    df_comb, meta = combine_trials_kuka_ati(
        CONFIG["kuka_csv"], CONFIG["ati_csv"], out_csv=out, plot=CONFIG["plot"]
    )
    print(f"Saved: {out}")
